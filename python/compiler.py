...
def generate_proto(source, output_dir, with_plugin='python', suffix='_pb2.py', plugin_binary=None):
    """
    ...
    if protoc is None:
        raise CompilerException("protoc not found. Make sure that it is in the path.")

    output = os.path.join(output_dir, os.path.basename(source.replace(".proto", suffix)))
    ...
    protoc_command = protoc + ' -I "%s" --%s_out="%s" "%s"' % (os.path.dirname(source), with_plugin, output_dir, source)
    if plugin_binary:
    ...
    if os.system(protoc_command) != 0:
        raise CompilerException("Error occurred while running protoc.")
    else:
